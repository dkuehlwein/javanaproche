...
messages_xmlmessages(
	[message(Importance, Type, Position, Subject, Description) | Messages],
	[element(message, [importance=Importance, type=Type, position=Position, value=Subject, repair=Description], [])
		| XmlMessages]
	) :-
	messages_xmlmessages(Messages, XmlMessages).
...
%
% @param Type is in {syntax, anaphor, ...}
% @param Position is the name of the predicate which failed
...
assert_message(Importance, Type, Position, Subject, Description) :-
%	ground(Importance),
%	ground(Type),
%	ground(Position),
%	ground(Subject),
	ground(message(Importance, Type, Position, Subject, Description)),
	!,
	% uncomment the next line for immediate print-out of the messages
	%write(message(Importance, Type, Position, Subject, Description)), nl,
	getval(message, Messages),
	setval(message, [message(Importance, Type, Position, Subject, Description) | Messages]).

assert_message(Importance, Type, Position, Subject, Description) :-
    format(user_error, 'Malformed message: ~q\n', [message(Importance, Type, Position, Subject, Description)]),
    !.
...
assert_message_once(Importance, Type, Position, Subject, Description) :-
	getval(message, Messages),
	member(message(Importance, Type, Position, Subject, Description), Messages),
...
%
% @bug there should be a SWI built-in for that
/*
myexclude(_Element, [], []).

myexclude(Element, [OtherElement | List1], List2) :-
